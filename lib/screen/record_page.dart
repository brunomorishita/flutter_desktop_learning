...
class RecordPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Widget getItems(_) {
      List<Widget> items = <Widget>[_buildRecSignal(), _buildMicButton()];

      if (_recordPageStore.recordingState == RecordingState.Start) {
        items.add(_buildTimerAnimation());
      }

      return Column(children: items).padding(vertical: 20).card(
          elevation: 10,
    ...
    return Observer(builder: getItems);
  }
  ...
  Widget _buildRecSignal() {
    ...
    ].toRow(mainAxisAlignment: MainAxisAlignment.center).padding(vertical: 20);
  }

  Widget _buildRedLightAnimation(bool blink) {
    Widget redCircle =
        Styled.widget().backgroundColor(Colors.red).constrained(width: 15, height: 15).clipOval();

    if (!blink) {
      return redCircle;
    }

    return BlinkWidget(children: <Widget>[
      redCircle,
      Styled.widget()
